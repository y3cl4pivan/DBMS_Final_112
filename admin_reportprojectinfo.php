...
<head>
    ...
    <title>Admin Report Project Info Search</title>
...
<body>
    <div class="container">
        ...
        <h1>Search for Report Project Information</h1>
        <form action="admin_reportprojectinfo.php" method="post">
            <div>
        ...
        if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $validCategories = ['reporter_id', 'be_reported_project_id', 'report_time', 'reason'];
            ...
            if ($category == 'report_time') {
                ...
                $sql = "SELECT * FROM public.REPORT_PROJECT WHERE DATE(report_time) = :search_query ORDER BY report_time DESC";
            } else {
                $sql = "SELECT * FROM public.REPORT_PROJECT WHERE $category ILIKE :search_query ORDER BY report_time DESC";
            }
            ...
            try {
                ...
                if ($results) {
                    ...
                    echo "<table>";
                    echo "<tr><th>Reporter ID</th><th>Reported Project ID</th><th>Report Time</th><th>Reason</th></tr>";
                    foreach ($results as $row) {
                        echo "<tr><td>" . $row['reporter_id'] . "</td><td>" . $row['be_reported_project_id'] . "</td><td>" . $row['report_time'] . "</td><td>" . $row['reason'] . "</td></tr>";
                    }
                    echo "</table>";
                } else {
        ...
        $allReportsQuery = "SELECT * FROM public.REPORT_PROJECT ORDER BY report_time DESC";
