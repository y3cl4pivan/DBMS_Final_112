...
<html>
    ...
    <body>
        ...
        echo "欲檢舉帳號id：";
        echo "<input type='text' name='be_reported_id'><br/><br/>";
        echo "檢舉理由：";
        ...
        if($_SERVER["REQUEST_METHOD"]=="POST"){
            $be_reported_id=$_POST["be_reported_id"];
            $reason=$_POST["reason"];
            ...
            if($stmt->execute()){
                ...
                $sql="Insert Into REPORT_ACCOUNT(reporter_id, be_reported_id, report_time, reason)
                VALUES(:reporter_id, :be_reported_id, :report_time, :reason)";
                $stmt = $pdo->prepare($sql);
                $stmt->bindValue(':reporter_id', $reporter_id, PDO::PARAM_STR);
                $stmt->bindValue(':be_reported_id', $be_reported_id, PDO::PARAM_STR);
